...
自动保存必须覆盖所有**关键状态变更节点**，任何节点的进程意外终止都不能导致该操作效果丢失：

| 触发事件 | 说明 |
|----------|------|
| 捕捉成功 | 虾米加入 party / box 后立即写盘 |
| 战斗结束（野外）| 结算 EXP + 死亡后立即写盘 |
| 深渊每层通过 | 含 EXP 结算 / 物品掉落 / 疫病应用后立即写盘 |
| 深渊疫病死亡结算 | 撤退或全灭时掷骰完成后立即写盘 |
| 词条工坊每次操作 | 重投 / 升阶 / 封印 / 解封后立即写盘 |
| 正常退出 | `q` 退出主菜单时 |
| 异常退出 | `KeyboardInterrupt` 捕获时 |

**不写盘的时机**：战斗单个回合（turn）内 —— 进程在战斗中途被 kill 时，当次战斗的状态会回滚到战斗开始前（即上次写盘点）。这是可接受的设计边界。

### 12.2 启动时自动续档（last_slot 指针）

为支持 agent 场景无人值守续档，引入 `last_slot` 文件：

- 路径：`~/.lobster-cli-tamer/last_slot`
- 内容：0/1/2（上次使用的槽位号）
- 写入时机：每次成功加载或新建存档后立即写入
- 启动行为：
  1. 检测 `last_slot` 文件是否存在
  2. 若存在且对应存档槽有效 → 显示摘要并询问「继续上次存档？(y/直接回车=yes / n=选择其他槽)」
  3. 若不存在 → 直接进入槽位选择菜单（现有行为）
- agent 默认行为：直接输入空行（回车）即可自动续档，无需 navigate 菜单

### Checklist
- [x] `save.py`：读写，3槽管理，version 字段，迁移预留
- [x] Memorial 纪念册写入（死亡时追加记录：虾米名/等级/词条/死亡层数）
- [x] 启动时存档槽摘要展示（含死亡数提示）
- [ ] **12.1 精确自动保存**：战斗结束、捕捉成功、深渊层通过后立即 `write_save`（已在部分路径实现，需补全 combat 内战斗结束回调）
- [ ] **12.2 last_slot 指针**：`save.py` 增加 `write_last_slot(slot)` / `read_last_slot()` → `game.py` 启动逻辑接入
